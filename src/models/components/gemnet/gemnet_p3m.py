...
from src.models.components.utils import InteractionBlock, MultiheadAttention
...
from .gemnet_utils import inner_product_normalized, ragged_range, repeat_blocks
...
class GemNet_P3M(nn.Module):
    ...
    def __init__(
        ...
        # Embedding block
        self.atom_emb = AtomEmbedding(emb_size_atom, max_z)
        ...
        for i in range(num_layers):
            a2m_mp = InteractionBlock(emb_size_atom, num_rbf, num_filters, grid_cutoff)
            m2a_mp = InteractionBlock(emb_size_atom, num_rbf, num_filters, grid_cutoff)  
        ...
        if self.regress_forces and self.direct_forces:
            self.m_mlp_rbf_out = Dense(num_rbf, emb_size_atom, activation=None, bias=False)
    ...
    def generate_interaction_graph(self, mesh_atom_graph):
        num_atoms = mesh_atom_graph['atom'].atomic_numbers.size(0)
    ...
    def forward(self, mesh_atom_graph):
        ...
        batch  = mesh_atom_graph['atom'].batch if hasattr(mesh_atom_graph['atom'], 'batch') else torch.zeros_like(mesh_atom_graph['atom'].atomic_numbers)
        ...
        # Embedding block
        a_x = self.atom_emb(mesh_atom_graph['atom'].atomic_numbers)  # (nAtoms, emb_size_atom
        ...
        if self.regress_forces:
            ...
            return energy, forces  # (nMolecules, num_targets), (nAtoms, 3)
        else:
            return energy, None  # (nMolecules, num_targets), None
