...
def compute_neighbors(batch, edge_index):
    # Get number of neighbors
    # segment_coo assumes sorted index
    ones = edge_index[1].new_ones(1).expand_as(edge_index[1])
    num_neighbors = segment_coo(
        ones, edge_index[1], dim_size=batch.size(0)
    )

    # Get number of neighbors per image
    image_indptr = torch.zeros(
        torch.max(batch) + 2, device=edge_index.device, dtype=torch.long
    )
    ones = batch.new_ones(1).expand_as(batch)
    image_indptr[1:] = segment_coo(
        ones, batch, dim_size=torch.max(batch) + 1
    )
