...
from ..utils import EquivariantVector, Scalar
from .painn_block import PaiNNMessage, PaiNNUpdate
from .painn_utils import RadialBasis
...
class PaiNN_P3M(nn.Module):
    def __init__(
        ...
        num_grids: Union[List, int],
        long_type: str = 'MHA', # 'FNO' 
        direct_forces=True,
    ):
        super().__init__()
        ...
        self.a2a_distance_expansion = RadialBasis(num_rbf, atom_cutoff)
        ...
        for _ in range(num_layers):
            ...
            short_mp = PaiNNMessage(hidden_channels, num_rbf)
            short_update = PaiNNUpdate(hidden_channels)
            if long_type == 'FNO':
        ...
        if self.regress_forces and self.direct_forces:
            self.a_forces_output = EquivariantVector(hidden_channels)
            self.m_rbf_proj = nn.Linear(num_rbf, hidden_channels, bias=False)
            self.m_forces_output = EquivariantVector(hidden_channels)
