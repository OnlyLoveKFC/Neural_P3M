...
def train(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    if hasattr(datamodule, "statistics"):
        datamodule.setup()
        results = datamodule.statistics()
        train_mean = results[0]
        train_std = results[1]
    else:
    ...
    if train_mean is not None and train_std is not None:
        model: LightningModule = hydra.utils.instantiate(cfg.model, mean=train_mean, std=train_std)
    else:
    ...
    if cfg.get("train"):
        ...
        if cfg.get("ckpt_path"):
            ckpt_path = list(Path(cfg.get("ckpt_path")).rglob('last.ckpt'))
            ckpt_path.sort()
            if len(ckpt_path) == 0:
                ckpt_path = None
                ...
                log.info(f'Resuming training from last checkpoint {str(ckpt_path)}!')
        else:
            ckpt_path = None
